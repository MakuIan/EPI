...
def initialize_variables(loaded_players: list[str]) -> tuple[dict[str, int], dict[str, list]]:
    '''
    ...
    players_map = ue04.hand_out_cards(
        ...
        card_list, loaded_players, len(card_list) // len(loaded_players))
    return points, players_map


def play_game(players: list, scores: dict[str, int], all_hands: dict[str, list]) -> dict[str, int]:
    '''
    ...
    def update_score(won_cards: list, player: str, scores: dict[str, int]) -> dict[str, int]:
        '''
        Updates the score of the player. Gives one penalty point for every card with
        the suit "Kreuz". 
